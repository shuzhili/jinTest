...
int deal_cache_setLifecycle(cJSON *root){
    ...
    redisContext* conn=rop_connectdb_nopwd(g_db_config.cache_ip,g_db_config.cache_port);
    if(conn==NULL){
        LOG(LOG_MODULE_SERVER_DATA,LOG_PROC_CACHE,"redis connnect error %s:%s",g_db_config.cache_ip,g_db_config.cache_port);
        ret=-1;
        goto END;
    }
    ret=rop_set_key_lifecycle(conn,key->valuestring,lifecycle->valueint);
END:
    rop_disconnect(conn);
    return ret;
}
...
char* deal_cache(const char *request_data_buf){
    ...
    if(strcmp(cmd->valuestring,KEY_CMD_SETSTRING)==0){
        ...
        RVALUES  rvaluse=NULL;
        int value_num=0;
        ret=deal_cache_getHash(root,&ralues,&value_num);
        response_data=make_response_gethash_json(ret,"get hash filed error",rvaluse,value_num);
        if(rvaluse!=NULL){
            free(rvaluse);
        }
